...
int main(int argc, char** argv) {
	...
	if(arg_select(argc, argv, &argselect, hash)) {
		struct bpkg_query qry_all_hashes = { 0 };
		struct bpkg_query qry = {0};
		struct bpkg_query qry_chunk_hashes = {0};
		...
		if(argselect == 1) {
			qry_all_hashes = bpkg_get_all_hashes(obj);
			

			
			// bpkg_get_all_chunk_hashes_from_hash(obj, 
			// 		hash);
			bpkg_print_hashes(&qry_all_hashes);
			
			bpkg_query_destroy(&qry_all_hashes);
			...
			qry_chunk_hashes = bpkg_get_all_chunk_hashes_from_hash(obj, 
					hash);
			bpkg_print_hashes(&qry_chunk_hashes);
			bpkg_query_destroy(&qry_chunk_hashes);
